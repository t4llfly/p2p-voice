        ...
        match status {
            Ok(updater) => match updater.get_latest_release() {
                Ok(release) => {
                    ...
                    info.download_url = Some(
                        release
                            .assets
                            .iter()
                            .find(|a| a.name.contains(".exe") || a.name.contains(".zip"))
                ...
                Err(e) => {
                    println!("Не удалось проверить обновления: {}", e);
                }
            },
            Err(e) => {
                println!("Ошибка конфигурации updater: {}", e);
            }
        }
    });
}
...
pub fn perform_update() -> Result<(), String> {
    ...
    match status.update() {
        ...
        Ok(self_update::Status::UpToDate(_version)) => Ok(()),
